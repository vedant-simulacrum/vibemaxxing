...
| Codex CLI — OTLP metrics | The `[otel]` configuration block in the CLI's own configuration file | Metric `codex.turn.token_usage` | Consumer account | Yes for the interactive surface only | `openai/codex#12913`: `codex exec` emits zero metrics and `codex mcp-server` emits nothing. Those two surfaces are silent, so any total derived from this adapter is missing whatever ran through them | **PARTIAL.** Deterministic for the interactive surface; a real and unclosable coverage gap for `exec` and MCP-server execution |
