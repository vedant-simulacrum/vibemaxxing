...
The adapter binds the registered accounting profile `cloud-separate-cache-v1` in `conformance/accounting/accounting-profiles-v1.json`. That profile is the shape of this source in every respect but one: four mutually exclusive categories, separate cache read and write, no source total, reasoning unavailable, modality unavailable, and a `quarantine` contradiction policy. PF-041 owns any narrower profile; a narrower profile changes the bound identity through the adapter manifest, not through this document.

**The one respect it is not.** That profile declares every one of its four source fields `provider-reported`, and section 7 of this document forbids recording `provider-reported` as the count authority for this capture. Both statements are about the same number and only one can be right. Section 7 is: the collector never sees a provider response, it sees an unsigned counter the CLI process emits, and D-077 keeps every locally observed mechanism attested-local while D-100 records that no provider offers an individual-account attestation path. `cloud-separate-cache-v1` is a generic profile for a caller reading a provider's own usage object, which genuinely is provider-reported; applying it here imports that authority into a capture that cannot carry it. This is recorded as the `otel-count-authority` contradiction in `packages/schemas/accounting-profile-otel-v1.json` and is owned by `A-001`, because repairing it means registering a narrower profile and re-pointing the binding, which changes what a future certification certifies. Until then the event carries `runtime-reported` and the profile identity carries the overstatement.
