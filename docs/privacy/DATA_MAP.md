...
The coarse availability projection rides on `RankEntry` as well as on `PresenceLease`, and `RankEntry` is rendered on the universally-public global board. The claim above still holds, and it holds for a reason worth stating rather than by luck: `profiles.presence_visibility` admits `authorized-viewers` and `private` and no third value, so no participant can make their presence public, and an anonymous reader is authorized by neither. The global-board surface in `packages/schemas/projection-authorization-v1.json` evaluates `presence-visibility`, which for a reader with no identity resolves to the suppressed value — `offline`, the same value an inactive participant reads. Adding a `public` value to that column would make this row false, which is why it is recorded here as the thing that would have to change.
...
**A shared cache is not on that list, and the API now says so.** A recipient the record does not name is a recipient nobody assessed, and until PF-033 `packages/schemas/openapi-v1.yaml` declared no cache directive on any response, so a proxy, a content delivery network or a browser back-forward cache could hold a viewer-relative response and hand it to a second viewer. `x-response-cache-policy` classifies every operation: `no-store` by construction, and `public-shared` only for the operations `x-public-operations` already gives the `global-board` or `reference-data` reason, both of which return a body identical for every reader. The `auth-bootstrap` operations are deliberately not shared-cacheable — they are public because they establish a session, not because their bodies are, and they carry authorization codes and session handles.
