...
| Raw Token Burn total per claim | Derived on the device, submitted | Yes | 6(1)(a) consent | As above | Hosting processor. The aggregate is **public** at period granularity |
...
| Period scores, generations and snapshots (`period_scores`, `score_snapshots`) | Derived server-side | Server-side only | 6(1)(a) consent | Retained as historical standing for the life of the ranked identity | Hosting processor; **public** for the global board |
...
| The public | Not a recipient in the Article 4(9) sense, but the effect is the same | Handle, Token Burn score at period granularity, rank, Estimated Cash Burn, awarded evidence profile, and any board membership the participant chose to make public | Worldwide |
...
Every window above is stated in real units. The windows are the controller's decision and this document owns them until PF-050 moves them into `packages/schemas/policy-defaults-v1.json` and its data-disposition companion, at which point the machine-readable form governs and this table summarises it.

Backups and point-in-time recovery archives are retained 35 days. That is the outer bound on how long an erased row can survive anywhere, and it is why the erasure completion statement in `PRIVACY.md` is 30 days for live systems and 35 days for backups rather than "immediately". Restores reapply deletion tombstones before serving traffic, which `docs/operations/DATA_LIFECYCLE_AND_RECOVERY.md` requires and which no restore drill has yet demonstrated, because nothing is provisioned.

## Erasure, and the conflict it has not resolved
...
D-085 is `provisional` and the reason is recorded there rather than hidden: that outcome conflicts with the binding rule that accepted claims and historical facts are immutable and that corrections and deletions are append-only, and with the immutable retained ranking generations and durable cursors the schema inventory requires. **The erasure outcome is decided. The mechanism that reconciles it with immutable generations is not, and this map does not invent one.** PF-029 and PF-022 own the choice between rebuilding under a new generation identifier and a tombstone-with-suppression model.

Article 17(2) obliges the controller, having made the data public, to take reasonable steps including technical measures to inform other controllers processing it that the subject has requested erasure. For a public leaderboard that means search-engine caches, archive services, and any mirror. The reasonable steps the controller commits to are: removal from the live surface within 30 days, removal from backups within 35 days, `noarchive` and removal-request submission to the search engines that expose a removal interface, and an honest statement in `PRIVACY.md` that a third party who copied a standing before erasure is outside the controller's reach.
