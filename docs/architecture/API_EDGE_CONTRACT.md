...
The `x-idempotency-contract` block at the root of `packages/schemas/openapi-v1.yaml`, landed by PF-049 under D-225, states the wire half in full: the key is scoped to `(principal_type, principal_id, operation_id, idempotency_key)`, the committed response is recorded whole rather than as a digest, a replay returns it byte-identically and carries `Idempotency-Replayed`, a same-key-different-digest request is `409` with `IDEMPOTENCY_KEY_CONFLICT`, and a record aged past its 168-hour replay window is `410` with `IDEMPOTENCY_RECORD_EXPIRED`.

**The persistence half is repaired, which is not the same as SR-012 being closed.** The paragraph that stood here said `planning-schema.sql` still stored a nullable `response_digest` with no response-body column and keyed the ledger on the account alone; PF-019 keyed it on the declared scope, PF-049 gave it `response_status`, `response_body` and `response_digest` with the two constraints that make a replayable row answerable, and PF-020 split the replay window from the row's own retention so an expired key is refused rather than becoming a fresh mutation. The excuse outlived the hole by two units, which is the only reason it is described here rather than deleted. A client may now treat byte-identical replay as specified *and* storable; it is still not demonstrated, because no PostgreSQL instance in this repository has executed any of it. `conformance/p1140f/semantic-findings-v1.json` is the only authority for the finding's state, and this document is not it.
