...
Verifiers compare the cofactored equation rather than recomputing `R'` and comparing byte strings, and carry no ad-hoc excluded-point list. **Neither language's default satisfies this.** Go's `crypto/ed25519` is cofactorless. So is `ed25519-dalek`: its `verify` and `verify_strict` recompute `R'` and compare it to the signature's `R`, which is the cofactorless rule, and the only thing that crate does under ZIP-215 is decode points — `VerifyingKey::from_bytes` documents "verifies the point is valid under ZIP-215 rules", and there is no ZIP-215 verification mode on it. An earlier revision of this paragraph claimed there was, and that claim was the reason Rust looked exempt from a constraint D-192 states in language-neutral terms. Both verifiers must use a ZIP-215-capable implementation — `ed25519-zebra` 2.x and later implement the post-ZIP-215 criteria in Rust, `github.com/hdevalence/ed25519consensus` in Go — and selecting them is part of the D-012 bakeoff rather than an implementation detail. `ed25519-dalek` remains suitable for signing.

Conformance requires divergence-case vectors, not RFC 8032's own test vectors — those pass under every implementation and therefore prove nothing. The required cases are non-canonical `A` and `R` encodings, small-order `A`, and `S ≥ ℓ`. `conformance/vibeproof/v1/ed25519-divergence-corpus.json` carries them: nine cases covering those four axes plus a well-formed control and a case in which every byte is canonical and the two criteria still disagree, generated by `scripts/repository/generate_ed25519_divergence_corpus.py` and byte-reproducible under its `--check` mode. Each case records the ZIP-215 verdict and, where they differ, the cofactorless one, and no verdict is asserted from this document: every ZIP-215 verdict is confirmed against `github.com/hdevalence/ed25519consensus`, with the run and its provenance recorded in `conformance/vibeproof/v1/zip215-oracle-run.json` and the generator refusing to write a corpus its oracle contradicts. `conformance/vibeproof/v1/README.md` states what the corpus proves and does not. D-340 through D-344 record the construction, the confirmation rule, and the measured finding that two deployed cofactorless verifiers accept a non-canonical `A` encoding RFC 8032 §5.1.3 says must fail to decode.

The corpus is a precondition of any cross-language conformance claim and is not itself one. No implementation consumes it: P-003 and P-004 are the units that make the Rust and Go verifiers yield its recorded verdicts, and selecting the ZIP-215-capable Go implementation the product ships remains part of the D-012 bakeoff. `ed25519consensus` is used here as a measuring instrument, not adopted as a product dependency.
