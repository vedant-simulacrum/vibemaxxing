...
| The root of one interface's migration chain | Interface, rows with no predecessor only | `storage_migrations_chain_root_idx`, unique and partial. It is the one-chain-per-interface rule rather than a lookup: `unique (interface, predecessor_version)` refuses a fork, and NULL is distinct from NULL in a unique constraint, so without this index "no predecessor" was the one value any number of rows could hold and an interface could have two beginnings |
| Walking a migration chain backwards | Step to the step before it | `storage_migrations_predecessor_idx`. `unique (interface, predecessor_version)` leads on the interface and supports no lookup by predecessor alone |
