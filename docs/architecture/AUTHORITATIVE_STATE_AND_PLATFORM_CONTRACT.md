...
- Sub-entity vocabularies that are not aggregate lifecycles are declared in `SQL_LOCAL_VOCABULARIES` in the validator: `device_keys.state`, `quarantines.state`, `deletion_effects.state`, `platform_certifications.state`, `appeal_decisions.decision`, `notification_deliveries.state`, `local_deletion_commands.disposition`, `local_deletion_receipts.outcome`, the assessed `provenance_state`/`continuity_state`/`integrity_state` pair on `evidence_assessments` and `verifier_appraisals`, `evidence_assessments.public_state`, and and the lineage `continuity_state` on `device_lineages`, which is its sole owner under PF-009: it previously sat on `device_sequences` as well, with nothing stating which of the two won.
