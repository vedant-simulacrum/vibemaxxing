...
**Maturity.** This inventory records declared ownership. It is not evidence that any listed contract is implemented, exercised, correct, or launch-ready. `scripts/repository/validate_planning_coverage.py` resolves this file against `packages/schemas/` and `conformance/` in both directions: every citation into either tree must resolve to a file, a directory or a pattern that matches something, and every file this repository ships under either tree must be owned by a row here or by a check that enumerates its directory. Passing that establishes declared coverage only, not implementation evidence. `packages/schemas/README.md` is that tree's own index and is subordinate to this file: where the two disagree about who owns a contract, this one is the authority and the other is the defect.

Two directories are enumerated elsewhere rather than listed row by row, because a second list of the same files is a second owner for one vocabulary. `packages/schemas/examples/` is enumerated by `validate_schema_example_coverage` in `scripts/repository/validate_planning_artifacts.py`, which reads the directory, refuses a file whose prefix no schema owns, and executes each `.valid`/`.invalid-` expectation. Each `conformance/<suite>/` that declares a `manifest.json` is enumerated by `validate_conformance_manifests` in the same file, which refuses a suite holding a file no case, authority or tooling entry names and recomputes every recorded fixture digest. The delegation is checked: the coverage validator fails if either function stops existing.
...
| Appraisal policy bundle | evidence contract | `packages/schemas/appraisal-policy-v1.schema.json`; `packages/schemas/appraisal-policy-v1.json` | present-provisional | SR-017; D-268; PF-043. Binds `packages/schemas/evidence-profile-policy-v1.json` by digest, adds wire ordinals, the D-078 E1 limb refinement, validity, supersession and the SQL binding. Provisional until a verifier implementation digest exists to pin |

## Identity, authentication, device and continuity specifications

| Specification family | Normative owner | Machine owner | Status | Repair / implementation dependency |
|---|---|---|---|---|
| Provider capability and OAuth transactions | `docs/security/AUTHENTICATION_AND_RECOVERY.md` | OpenAPI, PostgreSQL, state registry; `packages/schemas/oauth-provider-registry-v1.json`; `packages/schemas/oauth-provider-registry-v1.schema.json` | present-blocked | SR-006; PF-007..PF-010; O epic. The provider registry was recorded here as *proposed* long after it was authored, which is the drift the bidirectional coverage check now refuses in the other direction too |
...
| Device installation, key and lineage | device/evidence contracts | `packages/schemas/device-lineage.schema.json`; OpenAPI; SQL; state registry | present-blocked | SR-007; PF-011..PF-016; D epic |
...
| Specification family | Normative owner | Machine owner | Status | Repair / implementation dependency |
|---|---|---|---|---|
| Daemon, collector, sync, shell and CLI boundaries | native runtime contracts; ADR-010/012/013 | `packages/schemas/local-control-v1.proto`; `packages/schemas/local-trust-domains-v1.json`; `packages/schemas/local-trust-domains-v1.schema.json`; platform/state registries | present-blocked | SR-008; PF-017..PF-019; N epic |
...
| Local IPC handshake and capabilities | native runtime contract | `packages/schemas/local-control-v1.proto` | present-blocked | OS peer, artifact identity, generation, daemon-assigned role, nonce/sequence and revocation |
...
| Platform supervision | ADR-010/011/012 | `packages/schemas/platform-profile-registry-v1.json`; `packages/schemas/platform-profile-registry-v1.schema.json` | present-blocked | exact OS mechanisms, restart guarantees and honest weaker-profile labels |
| Presence pulse and visibility | product/privacy contracts; D-073 | `packages/schemas/presence-pulse-v1.schema.json`; `planning-schema.sql` `presence_leases`, `presence_events`, `profiles`; `conformance/social/presence-merge-vectors.json` | present-provisional | D-618 and D-619. Native-only qualifying pulses under device proof, a lease generation that stops a resumed process reviving an expired lease, visibility as an account-scoped policy rather than a per-device column, a stated multi-device merge evaluated under both orderings, and three thresholds bound to policy keys whose names now match their values. The two misnamed keys D-385 deferred are renamed. Provisional because no daemon emits a pulse and no surface renders a projection |

## Adapter, accounting and certification specifications

| Specification family | Normative owner | Machine owner | Status | Repair / implementation dependency |
|---|---|---|---|---|
| Adapter capability manifest | universal compatibility contract | `packages/schemas/adapter-manifest.schema.json`; `conformance/adapters/agent-registry-v1.json` | present-blocked | capability declaration must not imply certification |
...
| Accounting profile | accounting contract | `packages/schemas/accounting-profile.schema.json`; `conformance/accounting/accounting-profiles-v1.json` | present-provisional | PF-040; D-261/D-262. Profiles now carry `component_map` and real canonical digests, and the retry/cancel/nested enums have defined behaviour. Provisional because three planning profiles are registered and no certified source binds one |
| Accounting arithmetic | `docs/product/TOKEN_ACCOUNTING_SPEC.md` | `packages/schemas/accounting-arithmetic-v1.json`; `packages/schemas/accounting-arithmetic-v1.schema.json`; `packages/schemas/accounting-arithmetic-vectors-v1.schema.json`; `packages/schemas/reconciliation-vectors-v1.schema.json`; `conformance/accounting/arithmetic-vectors-v1.json` | present-provisional | PF-040; D-260/D-261/D-263. Domain, order, overflow, containment, rounding, digest and correction semantics; the planning validator recomputes every vector. Two independent language implementations remain outstanding |
| Multi-observer deduplication | accounting/integrity contracts | `packages/schemas/observer-equivalence-v1.json`; `packages/schemas/observer-equivalence-v1.schema.json`; `packages/schemas/dedup-vectors-v1.schema.json`; `conformance/accounting/dedup-vectors-v1.json` | present-provisional | PF-051; D-269. Server-owned preimage, three equivalence classes, exclusivity unit and survivor order across direct, proxy, ACP, OTel, subagent and live-log observation. Provisional until the key-derivation function is exercised rather than declared |
...
| OpenTelemetry accounting profile | universal compatibility contract | `packages/schemas/accounting-profile-otel-v1.schema.json`; `packages/schemas/accounting-profile-otel-v1.json`; `packages/schemas/producer-accounting-binding-v1.schema.json`; `packages/schemas/otel-capture-vectors-v1.schema.json`; `conformance/accounting/producer-bindings-v1.json`; `conformance/accounting/otel-capture-vectors-v1.json` | present-provisional | PF-041; D-264, D-614. The binding halves bind schema URL, semantic-conventions version, instrumentation scope, metric shape, reset detection and attribute disposition; this row previously listed only those and read as though the profile existed, which it did not. The profile now names one origin and one determinism class for every top-level field of `normalized-event.schema.json` and holds its supported metric set equal to the binding's declared metrics, so a metric cannot be claimed without a replayed capture. One metric is supported. `claude-code-otel-v1` is `candidate`, so its effective ceiling is `private-analytics`; the schema URL and conventions version are null because the measured producer emits neither, and the profile records two disagreements it does not repair — `otel-count-authority` and `otel-certification-bundle-absent` |
| SLM detector | D-053 | `packages/schemas/local-detector-result.schema.json` | post-launch | advisory only; cannot alter totals, evidence class or enforcement directly |

## Server state, ranking and social specifications

| Specification family | Normative owner | Machine owner | Status | Repair / implementation dependency |
|---|---|---|---|---|
| Public API | authoritative state contract | `packages/schemas/openapi-v1.yaml` | present-blocked | SR-006..SR-017; YAML under D-140; no generated production clients yet |
| PostgreSQL model | authoritative state contract | `packages/schemas/planning-schema.sql` | present-blocked | named owners, keys and vocabularies must align before migrations |
| Mutable-state registry | authoritative state contract | `packages/schemas/state-machine-registry-v1.json`; `packages/schemas/state-machine-registry-v1.schema.json` | present-blocked | every state reachable; every owner exists; no hidden mappings |
| Reason and policy registries | owning domain contracts | `packages/schemas/reason-codes-v1.json`; `packages/schemas/policy-defaults-v1.json` | present-blocked | add stable domain outcomes and accepted D-070..D-077 defaults |
| Ranking definition and audience | product/ranking contracts | `packages/schemas/ranking-view-v1.schema.json`; `planning-schema.sql` `ranking_definitions`, `ranking_views` | present-blocked | SR-010; the two halves are separate objects with disjoint property sets and their own digests, and `ranking_view_id` is the digest over both; `check ((scope = 'global') = (default_visibility = 'universally-public'))` puts public-global-only at the write site; PF-021 |
| Ranking generation, entries and snapshot | `docs/architecture/SERVER_API_DATA_AND_RANKING_CONTRACT.md`; `docs/architecture/LEADERBOARD_STORAGE_AND_RANKING.md` | `packages/schemas/ranking-generation-v1.schema.json`; `packages/schemas/ranking-cursor-v1.schema.json`; `packages/schemas/ranking-cursor-vectors-v1.schema.json`; `conformance/planning/ranking-cursor-vectors-v1.json`; `planning-schema.sql` `ranking_projection_generations`, `ranking_entries`, `score_snapshots` | present-blocked | D-211, D-212; generations are sealed once, one active generation per view is a partial unique index, and a cursor binds the viewer, the generation and an expiry; SR-015 remains open and PF-033 owns it |
| Period and season lifecycle | product/ranking contract | `packages/schemas/period-calendar-v1.schema.json`; `planning-schema.sql` `seasons`, `periods` | present-blocked | D-088; boundary order is a check-constraint chain from end through freeze, close, appeal window and archive, and `periods.state` now carries the open/frozen/closed/corrected/archived machine the chain was ordering nothing against; no calendar instance is populated |
| Score contribution and correction | accounting/ranking contracts; D-070 | `packages/schemas/score-contribution-v1.schema.json`; `packages/schemas/ranking-correction-vectors-v1.schema.json`; `conformance/planning/ranking-correction-vectors-v1.json`; `planning-schema.sql` `score_contributions`, `ranking_corrections` | present-blocked | claim-level explainability survives an erasure with the claim reference cleared; consolidation carries claim-level deltas rather than summed totals; the ledger is append-only by trigger and a correction names its period and participant, so a rebuild can attribute it; PF-023 |
| Pricing datasets and cost interpretation | product/ranking contracts | `packages/schemas/pricing-interpretation.schema.json`; `conformance/pricing/`; `planning-schema.sql` `pricing_datasets`, `pricing_entries`, `cost_interpretations` | present-blocked | Estimated Cash Burn is always labelled estimated and server interpreted, so the dataset a figure was interpreted under is part of the figure. No dataset is populated and no interpretation has been run. This family had no row at all until PF-034 resolved the inventory against the tree and found the schema owned by nothing |
| Movement, overtake and streak events | product contract | `packages/schemas/ranking-event-v1.schema.json`; `planning-schema.sql` `ranking_movement_events` | present-blocked | each event cites two sealed generations; duplicate suppression is a unique constraint; retraction is appended |
| Friendship, directional block and rivalry | product/privacy contracts | OpenAPI/SQL/state; `packages/schemas/social-integrity-events-v1.proto` | present-blocked | separate aggregates and generations |
| Board membership, roles and ownership | product contract; D-071 | OpenAPI/SQL/state/events; `conformance/p1140e/sql-race-plans-v1.json` `board-create-owner` | present-provisional | D-616 and D-617. `board_invites` records its invitee and a role constrained to `member` and `viewer`, so the non-privileged-invitation rule is a constraint rather than a refusal comparing fields no record held. Promotion and transfer stay separate and recent-authenticated, and the transfer's demotion is a declared transition. Neither board machine carries a block-caused terminal state. Provisional because no handler creates a board and the creation transaction is a plan rather than an execution |
| Notifications | `docs/product/SOCIAL_INTEGRITY_AND_UX_CONTRACT.md`; D-086 | `packages/schemas/notification-delivery-v1.schema.json`; `planning-schema.sql` `notification_events`, `notifications`, `notification_deliveries`, `notification_preferences`; `notification-delivery` machine | present-provisional | D-420..D-423. The source event, inbox item, transport attempt and preferences are four records in one file. Deduplication is a unique constraint over the outbox pair rather than worker logic, grouping is a reproducible digest, hysteresis binds the two overtake policy keys, and the four pre-inbox states are internal. A `server-inbox` attempt has one outcome, a push or email attempt needs the opt-in that authorized it, and no attempt of any transport carries a read. Every event type names the preference category that governs it under D-620, `product_enabled` covers the two that named none, and `notification_preferences` has the two API operations it previously lacked. Provisional because no aggregate appends an event and no surface renders an inbox, and because D-086 leaves the day-7 retention mechanism dependent on the participant opening the product |
...
| Specification family | Normative owner | Machine owner | Status | Repair / implementation dependency |
|---|---|---|---|---|
| Privacy boundary and egress | privacy contracts | `packages/schemas/egress-allowlist-v1.json`; `packages/schemas/egress-allowlist-v1.schema.json`; `packages/schemas/observability-allowlist-v1.yaml`; canary fixtures | present-blocked | all new server/native records require classification and egress review |
| Current viewer authorization | privacy contract | `packages/schemas/projection-authorization-v1.schema.json`; `packages/schemas/projection-authorization-v1.json`; `packages/schemas/authorization-invalidation-vectors-v1.schema.json`; `conformance/planning/authorization-invalidation-vectors-v1.json` | present-provisional | SR-015; D-386, D-621..D-624. Nine inputs each naming a table and a revision column the validator resolves, a fixed deny-before-widen order, no authorization-result cache, revision recheck and fail-closed. PF-033 made it total over the API: one boundary per operation compared for equality against the document, each boundary's subject computed in both directions, ten surfaces each partitioning all nine inputs into evaluated and excused, a derived per-field gate matrix asserted against the OpenAPI document joined with the disclosure projection, and one invalidation case per trigger evaluated rather than read back. This names the rule; no surface evaluates it, so SR-015 is advanced and remains open |
| Export request, snapshot and package | privacy/product contracts | `packages/schemas/export-manifest-v1.schema.json` plus proposed request/snapshot/artifact/grant schemas | present-blocked | SR-013; coherent encrypted self-describing export |
| Hosted deletion plan/effects | `docs/privacy/ERASURE_AND_KEY_DESTRUCTION.md`; ADR-022 | `packages/schemas/erasure-record-v1.schema.json`; `planning-schema.sql` `erasure_keys`, `erasure_domains`, `erasure_records`, `erasure_restore_receipts`, `deletion_tombstones` | present-blocked | D-085, D-210..D-214; SR-013 remains open and this supplies its deletion and backup half only; no restore drill has been run |
...
| Data disposition registry | privacy contract | `packages/schemas/data-disposition-v1.json`; `packages/schemas/data-disposition-v1.schema.json` | present-blocked | D-216; one row per persistence owner, every window resolving to a policy key or a named rule, every `expires_at` naming its actor; no sweeper exists |
...
| Telemetry and logs | privacy/operations contracts | `packages/schemas/observability-allowlist-v1.yaml` | present-blocked | fixed allowlist, no content-derived values and bounded retention |
...
| Release manifest and components | operations contract | `packages/schemas/release-set-v1.schema.json` | present-blocked | authenticated target, component IDs, paths, hashes, provenance and native signing |
...
| Deployment and operations | operations contract | future infrastructure and runbook artifacts | planned-missing | deferred deliberately: gated after P-1104, and no deployment automation may exist during planning, so the artifact cannot be written without breaking the phase rule |
| Open-source release | `docs/operations/OPEN_SOURCE_RELEASE_CHECKLIST.md`; D-040, D-540 | release-set schema; future NOTICE, SECURITY.md and CONTRIBUTING.md | present-provisional | seven items, all unmet and each naming what would satisfy it; item 1 is the LGPL attribution D-541 found open |
...
| Local development environment | `docs/engineering/LOCAL_DEVELOPMENT.md`; D-264 | proposed `compose.yaml` and `make` targets | planned-missing | deferred deliberately under D-264: the file lands with the first service. Its declared owner does not exist, which is what planned-missing means here rather than a dangling citation |
...
| Conformance harness | `docs/verification/CONFORMANCE_HARNESS.md`; D-242 | `packages/schemas/conformance-manifest-v1.schema.json`; sixteen `conformance/*/manifest.json` | present-provisional | D-441, D-442. Every suite directory outside `p1140e`, `p1140f` and `planning` declares a manifest and a README, and `scripts/repository/validate_planning_artifacts.py` resolves every authority, clause, fixture and digest in them. Fourteen of the sixteen declare `runner.state: absent`; two hold no fixture at all; three record a `negative_case_gap`. Every one of those four figures was one out of step when PF-034 recounted them, because the sixteenth suite arrived and the sentence stating how many there were did not move. The two that declare a runner both name `scripts/ci/run_phase1_evidence.py`, which reads two conformance files in total and emits none of the result document the harness contract defines. `conformance/vibeproof/v1/` is the normative corpus and no runner has ever executed it, which the `protocol` manifest now records against itself. The row cited D-262 before this change, which is the accounting-profile enum decision and not this one |
...
| Privacy/evidence disclosures | product/privacy contracts | `packages/schemas/disclosure-projection-v1.schema.json`; `packages/schemas/disclosure-projection-v1.json` | present-provisional | D-393, D-613. Every property of nine API schemas classified by audience and by the observed-versus-derived line D-108 uses, resolved against the OpenAPI document, with no field permitted to be narrower than the shape carrying it and the four D-144 figures pinned to `self` by name. `Relationship` was added by PF-033 under D-623: it was the one schema reachable from a success response that named an account other than its own subject and that this record classified nowhere. Provisional because no client reads it; `packages/ui` remains fixture-backed |
...
## Repository record and planning-projection specifications

These are the records this repository keeps about itself. They are contracts in the same sense as the rest — schema'd, validated and cited by tooling — and none of them had a row until PF-034 read the tree back against this file. A record with no owner is the one most likely to be edited to agree with whoever is reading it.

| Specification family | Normative owner | Machine owner | Status | Repair / implementation dependency |
|---|---|---|---|---|
| P-1140E structural evidence record | `docs/planning/P1140E_FINAL_CONTRADICTION_AUDIT_2026-07-24.md` | `conformance/p1140e/validation-matrix-v1.json`; `conformance/p1140e/validation-matrix-v1.schema.json`; `conformance/p1140e/state-machine-fixtures-v1.json`; `conformance/p1140e/platform-validation-plan-v1.json`; `conformance/p1140e/sql-race-plans-v1.json` | present-provisional | PF-035. The matrix holds two different kinds of thing: `decision_bindings` and `validation_domains` are a closed gate's evidence, while `api_operations`, `state_machines` and `platform_profiles` are projections of live registries and are regenerated by `scripts/repository/generate_p1140e_coverage.py`. Provisional because every fixture and plan in the directory declares `planned-runtime-evidence`: nothing here has been executed |
| P-1140F semantic review record | `docs/planning/P1140F_SEMANTIC_REVIEW_AND_STANDARDS_MAPPING.md` | `conformance/p1140f/semantic-findings-v1.json`; `conformance/p1140f/semantic-findings-v1.schema.json`; `conformance/p1140f/semantic-fixtures-v1.json`; `conformance/p1140f/artifact-authority-v1.json`; `conformance/p1140f/artifact-authority-v1.schema.json`; `conformance/p1140f/contract-bundles-v1.json`; `conformance/p1140f/contract-bundles-v1.schema.json`; `conformance/p1140f/gate-authorization-v1.json`; `conformance/p1140f/gate-authorization-v1.schema.json`; `conformance/p1140f/review-target-v1.json`; `conformance/p1140f/review-target-v1.schema.json`; `conformance/p1140f/REPAIR_HEAD_REVIEW.md` | present-blocked | SR-016; PF-036. `gate-authorization-v1.json` is the sole authority for phase and gate state and `AGENTS.md` restates it. `review-target-v1.json` is not pinned and no verdict is recorded, which is the finding rather than an omission to tidy: the pin and the verdict are an owner's act and no agent may write either |
| Derived planning record projections | `docs/planning/DECISION_REGISTER.md`; `docs/planning/TASK_CATALOG.md` | `conformance/planning/decisions-v1.json`; `conformance/planning/decisions-v1.schema.json`; `conformance/planning/decision-traceability-v1.json`; `conformance/planning/decision-traceability-v1.schema.json`; `conformance/planning/tasks-v1.json`; `conformance/planning/tasks-v1.schema.json` | present-provisional | These are machine projections of the two planning registers, regenerated by `scripts/repository/generate_planning_docs.py`. `conformance/planning/` declares no `manifest.json` and is not a conformance suite; it is the one directory under `conformance/` whose files are owned by this table row by row rather than by a suite manifest |
