...
|---|---|---|---|---|
| Adapter manifest | `packages/schemas/adapter-manifest.schema.json` | JSON Schema + updated positive fixture | P-1140B candidate repaired: artifact/manifest/provenance/SBOM digests, exact certification, accounting profiles and capability ceiling are closed-world | P-1140B validation |
| Local data stages | `source-observation.schema.json`, `normalized-event.schema.json`, `local-detector-result.schema.json` | JSON Schema + positive/negative examples | P-1140B candidate repaired: L0/L1 roles, retention/direction, typed counts/time/dedup/rules and `network_eligible=false` are explicit | P-1140B validation |
...
| Local IPC | `packages/schemas/local-control-v1.proto` | Protobuf compilation + opaque-payload guard | P-1140B candidate repaired: typed observation/ack/claim/queue/receipt/export/deletion bodies with role, nonce, sequence and deadline | P-1140B validation; protocol bytes in P-1140C |
...
| Privacy egress | `egress-allowlist-v1.*`, `conformance/privacy/p1140b-boundary-canaries-v1.json`, observability allowlist | JSON Schema/registry plus positive/negative boundary coverage | P-1140B claim egress candidate repaired; P-1140D must extend typed server/social/reviewer state boundaries without weakening it | P-1140B validation/P-1140D |
| Accounting profiles | `accounting-profile.schema.json`, `conformance/accounting/` | Schema, registry and representative cases | P-1140B candidate repaired: containment and mutually exclusive outputs cover cloud/local, cache, reasoning, retry, cancellation and contradiction | P-1140B validation |
| Evidence appraisal policy | `evidence-profile-policy-v1.json` | Parsed policy plus authority/downgrade validation | P-1140B candidate repaired: independent dimensions and server-owned downgrade order | P-1140B validation |
| Device lineage | `device-lineage.schema.json` | JSON Schema structural validation | P-1140B candidate repaired: enroll/rotate/recover/restore/clone/requalify dispositions | P-1140B validation; transaction state in P-1140D |
| Pricing interpretation | `pricing-interpretation.schema.json` | JSON Schema structural validation | P-1140B candidate repaired: immutable server-owned alias resolution and line items; claims have no pricing authority | P-1140B validation/P-1140D persistence |
