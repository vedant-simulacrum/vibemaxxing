...
- **The keyring becoming genuinely separable within budget**, which would shorten the restore window from "duration of the replay" to "zero". D-094's cost conflict is resolved by D-360, so this is now bounded by a measured number rather than by an unresolved contradiction — and that number does not exist until D-363's measurement is taken.
