...
function FriendRow({p,active=false}:{p:Person;active?:boolean}) { return <div className="friend-row"><div className="friend-name"><Avatar id={p.avatar} size={44}/><span><b>{p.name}</b><small>@{p.handle}</small></span></div><span className={active?"presence":""}>{active?"Burning now":"Last seen 1h ago"}</span><b>#{String(p.rank).padStart(2,"0")}</b><b>{p.burn}</b><ProductMovement value={p.change||1}/><Model name={p.model}/><span>3</span><span className="row-actions"><ProductButton>Compare</ProductButton><ProductButton>Rival</ProductButton><Ellipsis size={18}/></span></div>; }
