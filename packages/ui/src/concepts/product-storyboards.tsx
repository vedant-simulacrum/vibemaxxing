...
function Trend({ compare = false }: { compare?: boolean }) {
  const pts = compare ? "0,178 44,171 88,158 132,145 176,137 220,127 264,119 308,104 352,91 396,80 440,66 484,56 528,45 572,38 616,27 660,23 704,17" : "0,173 44,171 88,168 132,164 176,161 220,159 264,156 308,153 352,149 396,146 440,139 484,114 528,91 572,89 616,85 660,76 704,70 748,65 792,57 836,49";
  return <div className="vm-sb-trend"><svg viewBox="0 0 840 210" preserveAspectRatio="none"><defs><linearGradient id={`fill-${compare}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#4b20ff" stopOpacity=".13" /><stop offset="1" stopColor="#4b20ff" stopOpacity="0" /></linearGradient></defs>{[24,72,120,168].map(y => <line key={y} x1="0" y1={y} x2="840" y2={y} className="grid" />)}<polygon points={`${pts} 840,198 0,198`} fill={`url(#fill-${compare})`} /><polyline points={pts} className="line" />{compare && <polyline points="0,190 44,183 88,173 132,160 176,151 220,145 264,137 308,122 352,110 396,96 440,84 484,72 528,59 572,49 616,40 660,32 704,27" className="line rival" />}</svg><div className="vm-sb-dates"><span>Apr 22</span><span>Apr 29</span><span>May 6</span><span>May 13</span><span>May 20</span></div></div>;
}
...
export function FriendsStoryboard() {
  ...
  return <ProductShell active="Friends"><main className="vm-sb-content friends-page">
    ...
    <aside className="vm-sb-stack"><Panel className="request-card"><h2>Incoming requests <b>3</b><a>View all</a></h2>{people.slice(0,3).map((p,i)=><div className="request-row" key={p.handle}><Avatar id={p.avatar} size={38}/><span><b>{p.name}</b><small>{[8,5,6][i]} mutual friends</small></span><Button>Accept</Button><Button>Ignore</Button></div>)}</Panel>
...
export function BoardStandingsStoryboard() {
  const boardPeople=[people[0],people[1],people[2],{...people[6],rank:4,change:2},people[7],people[8],{...people[0],name:"Parker Zhao",handle:"parkerzhao",rank:7,burn:"72.4M",week:"419.3M",change:-2,avatar:4 as AvatarId},{...people[4],rank:8,burn:"68.9M",week:"402.8M",change:3},{...people[5],rank:9,burn:"65.3M",week:"384.1M",change:-1},{...people[3],rank:10,burn:"59.2M",week:"351.6M",change:2}];
