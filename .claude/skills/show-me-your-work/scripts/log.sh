...
#
# Portability: POSIX + bash 3.2 only, so it runs under macOS/BSD userland as
# shipped (no GNU coreutils, no flock, no `date -d`, no bash 4 features).
# Verified under /bin/sh and bash 3.2.57.
set -euo pipefail
...
# Write the header exactly once. noclobber makes the create-or-skip atomic, so
# two concurrent first-writers can't both emit a header row (macOS has no
# flock(1) to lock with).
if [ ! -f "$logfile" ]; then
	( set -C; printf 'ts\tphase\tdecision\twhy\tevidence\tresult\n' > "$logfile" ) 2>/dev/null || true
fi
if [ ! -f "$logfile" ]; then
	printf 'log.sh: cannot create %s\n' "$logfile" >&2
	exit 1
fi
