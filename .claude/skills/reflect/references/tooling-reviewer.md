...
Two other reviewers are reading the same transcript through a judgment lens and a divergent lens. All three of you are Anthropic models, so you do not have independent blind spots — the only thing making this a panel rather than one opinion repeated is that each of you holds a different lens. Hold yours. Leave the generalizable principles to the judgment reviewer and the contrarian framings to the divergent reviewer; go deeper on concrete mechanics than a generalist would.
...
- User pastes an issue title or PR body the agent could have fetched with `gh issue view` / `gh pr view`. Routing: the relevant skill should shell out to `gh` first.
- User describes a deploy failure the agent could have read from the Vercel or Cloudflare CLI/MCP. Routing: the deploy skill should query it directly.
- User quotes library API details the agent could have pulled from the docs MCP instead of guessing from training data. Routing: the relevant skill should call the docs MCP before writing integration code.
- User re-explains a decision from an earlier session that the memory/search MCP already holds. Routing: the skill should query memory before asking.

Prefer the CLI over an MCP when both exist and the CLI is more token-efficient (`gh`, `vercel`, `wrangler`); note which one the transcript actually used and whether the cheaper one would have worked.
...
- `Skill` tool calls, or `Read` calls against any `SKILL.md` (project `.claude/skills/`, user-level `~/.claude/skills/`, or plugin paths under `~/.claude/plugins/`)
- `<command-name>` blocks in user turns, which mean a skill or slash command was loaded
