---
name: blast-radius
description: "Use before shipping a risky change or reviewing a diff you don't trust: finds what it breaks elsewhere, beyond the diff, and proves the one safety fact by running real code instead of writing it up."
---
...
Companion to code exploration and archaeology. A locator agent (the `explorer` agent, or `Explore`) tells you what the code does. `git log`, `git blame`, and `gh pr view` tell you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commit history yourself: `git log --oneline -20 -- <file>`, `git blame -L <start>,<end> <file>`, and `gh pr view <number> --json title,body,comments,reviews` for anything substantive.
...
3. Look where grep stops. Read the source of the library you call, and check its pinned version and any local patch. Work out when things run: microtasks, unmount and teardown, one framework's lifecycle versus another's. Follow what a symbol search misses: the JSON an API returns, a DB column, a wire format, another language reading the same bytes, a feature flag, code three hops downstream. Delegate wide, read-only fan-out to the `explorer` agent or `Explore` when the search spans more files than you can hold in one pass.
...
6. For a big or wide change, fan out instead of relying on one pass: launch several Agent tool calls in a single message, each given the same question (or a distinct slice of the diff), then read every result and merge the answers by hand. Different agents catch different real bugs; merging is your job, not automatic.
